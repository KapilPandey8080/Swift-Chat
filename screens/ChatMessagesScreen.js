...
const ChatMessagesScreen = () => {
    ...
    const fetchMessages = async () => {
        try {
            const response = await fetch(
                `http://192.168.140.91:8000/messages/${userId}/${recepientId}`
            );
    ...
    useEffect(() => {
        const fetchRecepientData = async () => {
            try {
                const response = await fetch(
                    `http://192.168.140.91:8000/user/${recepientId}`
                );
    ...
    const handleSend = async (messageType, imageUri) => {
        try {
            ...
            const response = await fetch("http://192.168.140.91:8000/messages", {
                method: "POST",
                body: formData,
            });
            ...
            headerRight: () =>
                selectedMessages.length > 0 ? (
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                        ...
                        <Ionicons name="arrow-undo-outline" size={22} color="black" />
    ...
    const deleteMessages = async (messageIds) => {
        try {
            const response = await fetch("http://192.168.140.91:8000/deleteMessages", {
                method: "POST",
                headers: {
            ...
            <ScrollView ref={scrollViewRef} contentContainerStyle={{ flexGrow: 1 }} onContentSizeChange={handleContentSizeChange}>
                {messages.map((item, index) => {
                    ...
                    if (item.messageType === "image") {
                        const baseUrl =
                            "D:/Codes/Chat_App_React_Native/api/files/";
